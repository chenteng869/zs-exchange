...
test('secp256k1：从私钥 1 推导公钥 → address 已知', () => {
  ...
  // 注意：js-sha3 的 keccak256 接受 Buffer/Uint8Array（按字节）或字符串（按 UTF-8 文本），
  // 不会自动将 hex 字符串解码为字节，必须先转 Buffer。
  const hash = keccak256(Buffer.from(concat, 'hex'));
...
test('大额提现（> 5000 USD）：自动标记 high risk + pending_review', async () => {
  ...
  const svc = new WithdrawalService({
    ...
    getUsdPrice: async () => 2000, // 1 ETH = 2000 USD
  });
  // 5 ETH = 10000 USD > 5000 (high risk)，且 < 10 ETH 每日限额
  const amountWei = (5n * 10n ** 18n).toString();
  const req = await svc.createWithdrawalRequest(
    'user1', 'ETH', amountWei,
    '0x' + '1'.repeat(40), 'ETH',
  );
...
test('拒绝流程：rejectWithdrawal 设置 status=rejected', async () => {
  ...
  const svc = new WithdrawalService({
    broadcaster: wb,
    getBalance: async () => 10n ** 20n, // 100 ETH
    getUsdPrice: async () => 2000, // 5 ETH = 10000 USD -> high risk
  });
  // 用 5 ETH 制造 pending_review 状态（同时 < 10 ETH 每日限额）
  const req = await svc.createWithdrawalRequest(
    'user1', 'ETH', (5n * 10n ** 18n).toString(),
    '0x' + '1'.repeat(40), 'ETH',
  );
  assert.equal(req.status, 'pending_review');
...
test('WithdrawalService：listPendingApprovals 仅返回 pending_review', async () => {
  ...
  // 创建 3 个大额（5 ETH 各 10000 USD > high risk 阈值，每个用户 < 10 ETH 每日限额）
  for (let i = 0; i < 3; i++) {
    await svc.createWithdrawalRequest(
      'user' + i, 'ETH', (5n * 10n ** 18n).toString(),
      '0x' + i.toString(16).padStart(40, '0'), 'ETH',
    );
  }
  // 创建 1 个低额（0.01 ETH < 0.001? 实际 0.01 > minAmount，amountRaw=10^16 wei）
  await svc.createWithdrawalRequest(
    'lowUser', 'ETH', (1n * 10n ** 16n).toString(), // 0.01 ETH
