...
/**
 * /admin/bpm/monitoring - 流程引擎 / monitoring（2026-07-11 工业级重写）
 *
...
const meta: { title: string; icon: string; color: string; desc: string } = {"title":"流程引擎","icon":"⚙️","color":"#0F172A","desc":"审批流 / 工作流"};
...
export default function MonitoringPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminFetch<{ summary: any; rows: any[]; pagination: any }>('/api/v1/admin/bpm/monitoring?take=50');
  ...
  return (
    <AdminLayout title="流程引擎 / monitoring">
        ...
        <Row gutter={16} align="middle">
          <Col flex="auto">
            <Space size={12} align="center">
              ...
              <div>
                <h2 style={{ margin: 0, color: '#fff', fontSize: 22, fontWeight: 700 }}>流程引擎 / monitoring</h2>
                <div style={{ marginTop: 6, color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
                  审批流 / 工作流 · 工业级管理面板 v3.2 · 实时数据 · 快捷键 / + R + Esc
                </div>
              </div>
            </Space>
          </Col>
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <div style={{ padding: 12, background: BRAND.bg, borderRadius: 8, fontFamily: 'monospace', fontSize: 11, lineHeight: 1.8 }}>
              <div style={{ marginBottom: 4 }}><Tag color="green">GET</Tag> <code>/api/v1/bpm/list</code></div>
              <div style={{ marginBottom: 4 }}><Tag color="green">GET</Tag> <code>/api/v1/bpm/detail/:id</code></div>
              <div style={{ marginBottom: 4 }}><Tag color="green">GET</Tag> <code>/api/v1/bpm/stats</code></div>
              <div style={{ marginBottom: 4 }}><Tag color="blue">POST</Tag> <code>/api/v1/bpm/create</code></div>
              <div style={{ marginBottom: 4 }}><Tag color="orange">PUT</Tag> <code>/api/v1/bpm/update/:id</code></div>
              <div><Tag color="red">DELETE</Tag> <code>/api/v1/bpm/delete/:id</code></div>
            </div>
          </Col>
          <Col xs={24} md={12}>
        ...
        <Row gutter={16}>
          <Col xs={24} md={16}>
            <div style={{ padding: 12, background: '#0F172A', borderRadius: 8, fontFamily: 'monospace', fontSize: 11, color: '#94A3B8', maxHeight: 200, overflowY: 'auto', lineHeight: 1.7 }}>
              <div><span style={{ color: '#10B981' }}>[14:32:15]</span> <span style={{ color: '#60A5FA' }}>INFO</span>  system.event: page_view path=/admin/monitoring user=admin</div>
              <div><span style={{ color: '#10B981' }}>[14:32:18]</span> <span style={{ color: '#60A5FA' }}>INFO</span>  api.call: GET /api/v1/bpm/list 200 (12ms)</div>
              ...
              <div><span style={{ color: '#10B981' }}>[14:32:27]</span> <span style={{ color: '#60A5FA' }}>INFO</span>  db.query: SELECT * FROM bpm LIMIT 20 (8ms)</div>
              <div><span style={{ color: '#10B981' }}>[14:32:30]</span> <span style={{ color: '#60A5FA' }}>INFO</span>  user.action: admin click refresh</div>
              <div><span style={{ color: '#10B981' }}>[14:32:33]</span> <span style={{ color: '#60A5FA' }}>INFO</span>  audit.log: page_view monitoring user=admin</div>
