...
import {
  ChevronDown, Clock, BarChart3, AlertTriangle, Gauge, Shield, Wallet, Loader2,
} from 'lucide-react';
import {
...
export default function FuturesTradePage() {
  ...
  const marginRate = useMemo(() => ((1 / leverage) * 100).toFixed(2), [leverage]);
  const isHighLeverage = leverage >= 50;

  const handlePercentageClick = (percent: number) => {
    const availableBalance = 10;
    setAmount(((availableBalance * percent) / 100).toFixed(6));
  };
          ...
          <div className="max-w-[1600px] mx-auto px-4 h-full flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 px-3 py-1.5 bg-deep-900 rounded border border-white/10 hover:border-white/20 transition-colors">
                ...
                <span className="ml-1.5 text-[10px] px-1.5 py-0.5 bg-brand-500/15 text-brand-500 rounded font-medium">永续</span>
              </button>
            ...
            <div className="flex gap-1">
              {TIME_PERIODS.map((period) => (
                <button key={period} onClick={() => setTimePeriod(period)}
                  className={`px-3 py-1 text-xs font-medium rounded transition-colors ${timePeriod === period ? 'bg-deep-700 text-text-primary' : 'text-text-muted hover:text-text-primary hover:bg-deep-800'}`}>
                  {period}
                </button>
              ))}
            </div>
              ...
              <div className="bg-deep-800/50 border border-white/5 rounded-lg overflow-hidden">
                ...
                <div className="max-h-[180px] overflow-y-auto">
                  {(orderBook.asks.length > 0 ? [...orderBook.asks].reverse() : Array(10).fill(null)).map((ask, i) => (
                    <div key={`ask-${i}`} className="grid grid-cols-3 gap-2 px-3 py-1.5 text-xs relative hover:bg-white/[0.03] transition-colors">
                ...
                <div className="max-h-[180px] overflow-y-auto">
                  {(orderBook.bids.length > 0 ? orderBook.bids : Array(10).fill(null)).map((bid, i) => (
                    <div key={`bid-${i}`} className="grid grid-cols-3 gap-2 px-3 py-1.5 text-xs relative hover:bg-white/[0.03] transition-colors">
            ...
            <div className="col-span-6 space-y-4">
                ...
                <div className="p-4 h-[360px]">
                  {chartData.length > 0 ? (
                    <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#0F172A', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', color: '#F8FAFC', fontSize: '12px', padding: '8px 12px' }}
                        formatter={(v: number) => [`$${Number(v).toLocaleString()}`, '价格']} />
                      {amount && (price || orderType === 'market') && (
                        <ReferenceLine y={parseFloat(liquidationPrice)} stroke="#EF4444" strokeDasharray="6 4" strokeWidth={1.5}
                          label={{ value: `强平 $${liquidationPrice}`, position: 'right', fill: '#EF4444', fontSize: 10, fontWeight: 600 }} />
                      )}
                      <Area type="monotone" dataKey="price" stroke="#7C3AED" strokeWidth={1.5} fillOpacity={1} fill="url(#colorPriceFutures)" />
                    </AreaChart>
                  ) : (
              ...
              <div className="bg-deep-800/50 border border-white/5 rounded-lg overflow-hidden">
                ...
                <div className="divide-y divide-white/[0.03] max-h-[280px] overflow-y-auto">
                  {recentTrades.slice(0, 18).map((trade, idx) => (
                    <div key={idx} className="grid grid-cols-4 gap-2 px-3 py-1.5 text-xs hover:bg-white/[0.02] transition-colors">
                      ...
                      <span className={`font-mono font-medium tabular-nums ${trade.side === 'buy' ? 'text-emerald-500' : 'text-red-500'}`}>{trade.price.toFixed(2)}</span>
                      ...
                      <span className="text-right flex items-center justify-end gap-1">
                        ...
                        <span className={`[font-size:10px] ${trade.side === 'buy' ? 'text-emerald-500' : 'text-red-500'}`}>{trade.side === 'buy' ? '买' : '卖'}</span>
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-deep-800/50 border border-white/5 rounded-lg p-4">
                  ...
                  <div className="flex gap-2">
                    {(['isolated', 'cross'] as const).map((m) => (
                      <button key={m} onClick={() => setMarginMode(m)}
                        className={`flex-1 py-1.5 text-xs font-medium rounded transition-all duration-150 border ${marginMode === m ? 'border-brand-500/60 bg-brand-500/10 text-brand-500' : 'border-white/10 text-text-secondary hover:border-white/20'}`}>
                        {m === 'isolated' ? '逐仓' : '全仓'}
                      </button>
                    ))}
                  </div>
                ...
                <div className="mb-4 p-3 bg-deep-900/30 rounded-lg border border-white/5">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[11px] text-text-muted flex items-center gap-1.5"><Gauge size={13} />杠杆倍数</label>
                    <span className={`text-sm font-bold font-mono tabular-nums ${isHighLeverage ? 'text-red-500' : 'text-brand-500'}`}>{leverage}x</span>
                  </div>
                  <input type="range" min="2" max="100" value={leverage} onChange={(e) => setLeverage(Number(e.target.value))}
                    className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer mb-2 ${isHighLeverage ? 'accent-red-500' : 'accent-brand-500'}`} />
                  <div className="flex gap-1.5 flex-wrap">
                    {LEVERAGE_OPTIONS.map((lev) => (
                      <button key={lev} onClick={() => setLeverage(lev)}
                        className={`px-2.5 py-1 text-xs font-medium rounded transition-all duration-150 ${leverage === lev ? (isHighLeverage && lev >= 50 ? 'bg-red-600 text-white' : 'bg-brand-500 text-white') : 'bg-deep-800/50 text-text-secondary hover:text-text-primary border border-white/5'}`}>
                        {lev}x
                      </button>
                    ))}
                  </div>
                  {isHighLeverage && (
                    <div className="mt-2 p-2 bg-red-500/8 border border-red-500/15 rounded flex items-start gap-1.5">
                      ...
                      <span className="text-[10px] text-red-500 leading-tight">高杠杆风险: {leverage}x 杠杆将显著增加强平风险</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mb-4 p-3 bg-deep-900/30 rounded-lg border border-white/5">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5 text-text-muted"><Shield size={13} />初始保证金率</div>
                ...
                <div className="flex gap-2 mb-4">
                  {(['limit', 'market'] as const).map((t) => (
                    <button key={t} onClick={() => setOrderType(t)}
                      className={`flex-1 py-1.5 text-xs font-medium rounded border transition-all duration-150 ${orderType === t ? 'border-brand-500/60 bg-brand-500/10 text-brand-500' : 'border-white/10 text-text-secondary hover:border-white/20'}`}>
                      {t === 'limit' ? '限价' : '市价'}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2 mb-4">
                  {[25, 50, 75, 100].map((percent) => (
                    <button key={percent} onClick={() => handlePercentageClick(percent)}
                ...
                {(price || orderType === 'market') && amount && (
                  <div className="mb-4 space-y-2">
                    ...
                    <div className="p-3 bg-red-500/5 border border-red-500/15 rounded border-dashed">
                      ...
                      <div className="mt-1 text-[10px] text-text-muted">{orderSide === 'buy' ? '做多' : '做空'} · {leverage}x杠杆</div>
                    </div>
                  </div>
                )}
