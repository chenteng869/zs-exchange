// scan-yield-compliance.js
const fs = require('fs');
const path = require('path');

const ROOT = 'd:/3、系统项目开发/trae_projects/Stock Exchange dapp20260608-01';
const targets = [
  'src/components/portal-preview/PortalYield.tsx',
  'src/app/portal-preview/yield/page.tsx',
];

const FORBIDDEN = [
  '萨摩亚持牌', '萨摩亚牌照', '持牌交易所', '已取得监管', '已获得监管',
  'DSAEX-2024-001', 'MSA 监管', 'MSA 牌照', 'MSA', '持牌', '牌照',
  '承诺收益', '保本', '刚兑', '稳赚', '担保',
  '受监管保护', '已受监管', '受当地监管', '已获当地监管', '全球合法',
  '香港持牌', '新加坡持牌', '迪拜持牌', '马耳他持牌', '美国持牌', '塞浦路斯持牌',
  '已取得多国监管', '持牌身份', '监管许可', '已获许可',
  '绝对安全', '资产保证', '零风险', '保障本金', '无风险',
];

const NEG_RE = /(不|无|避免|规避|严禁|拒绝|反对|不构成|不提供|不做|不含|仅供|仅为|不是|不得)/;
...
for (const rel of targets) {
  ...
  console.log('=== ' + rel + ' total lines: ' + lines.length + ', size: ' + (fs.statSync(p).size / 1024).toFixed(1) + ' KB');
